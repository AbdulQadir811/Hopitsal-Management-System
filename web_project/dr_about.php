...
$conn = mysqli_connect("localhost", "root", "", "hospital");
  ...
  $id =$_SESSION['id'];
  $sql = "SELECT doctors.id,doctors.email,doctors.doc_name,doctors.blood_group,doctors.pass,doctors.age,doctors.sex,departments.name,doctors.birth_date,doctors.salary,doctors.fee,doctors.contact,doctors.address,doctors.shift FROM doctors INNER JOIN departments on doctors.dept_id = departments.id where doctors.id = $id ";
  ...
  if ($result->num_rows > 0) {
   ...
   while($row = $result->fetch_assoc()) {
    ...
    echo "<tr><td>" . $row["id"]. "</td><td>" . $row["email"]. "</td><td>" . $row["doc_name"]. "</td><td>". $row["blood_group"]. "</td><td>" . $row["pass"]. "</td><td>" . $row["age"] . "</td><td>" . $row["sex"] . "</td><td>" . $row["name"]. "</td><td>" . $row["birth_date"] . "</td><td>" . $row["salary"]. "</td><td>" . $row["fee"] . "</td><td>" . $row["contact"] . "</td><td>" . $row["address"] . "</td><td>" . $row["shift"] . "</td></tr>";
 }
echo "</table>";
} else { echo "0 results"; }
$conn->close();
?>
<form method='post' name="myform" action="dr_update.php">
