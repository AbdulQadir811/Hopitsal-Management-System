...
<!DOCTYPE html>
<html lang="en">
<title>Dr. <?php echo $_SESSION['name'];?> | dashboard</title>
<meta charset="UTF-8">
...
<form method='post' name="myform" action="patient_update_done.php">
...
//   $id = $_SESSION['id']; 
  ...
  if ($result->num_rows > 0) {
   // output data of each row
   while($row = $result->fetch_assoc()) {
    echo "<tr> <input  name=password type=text value =" . $row["pass"]. ">";}
echo "</table>";
} else { echo "0 results"; }
$conn->close();
?><br>
...
//   $id = $_SESSION['id'];
  ...
  if ($result->num_rows > 0) {
   // output data of each row
   while($row = $result->fetch_assoc()) {
    echo "<tr> <input type=text  name=contact value =" . $row["contact_no"]. ">";}
echo "</table>";
} else { echo "0 results"; }
$conn->close();
?><br>
...
//   $id = $_SESSION['id'];
  ...
  if ($result->num_rows > 0) {
   // output data of each row
   while($row = $result->fetch_assoc()) {
    echo "<tr> <input type=text  name=description value =" . $row["description"]. ">";}
echo "</table>";
} else { echo "0 results"; }
$conn->close();
?><br>
