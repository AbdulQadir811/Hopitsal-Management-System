...
<div class="w3-main" style="margin-left:340px;margin-right:40px">
  ...
  <div class="w3-container" style="margin-top:80px" id="showcase">
    ...
    <h1 class="w3-xxxlarge w3-text-gray"><b>Appointments View</b></h1>
...
$conn = mysqli_connect("localhost", "root", "", "hospital");
  ...
  $id = $_SESSION['id'];
  $sql = "SELECT appointments.id,doctors.doc_name,appointments.time,appointments.descriptions FROM appointments Inner JOIN doctors ON appointments.doctor_id = doctors.id  where appointments.patient_id= $id";
  
  $result = $conn->query($sql);
  if ($result->num_rows > 0) {
   // output data of each row
   while($row = $result->fetch_assoc()) {
    echo "<tr><td>" . $row["id"]. "</td><td>". $row["doc_name"]. "</td><td>" . $row["time"]. "</td><td>" . $row["descriptions"] . 
"</td></tr>";
 }
echo "</table>";
} else { echo "0 results"; }
